...
(function checkTransitionOnLoad() {
    ...
    if (transitionState === 'from_register' && mainCard) {
        ...
        const cleanup = () => { mainCard.classList.remove('page-transition-in-left'); mainCard.removeEventListener('animationend', cleanup); };
        mainCard.addEventListener('animationend', cleanup);
        setTimeout(cleanup, 700);
    } else if (mainCard) {
        mainCard.classList.add('animate-entry');
        const cleanup = () => { mainCard.classList.remove('animate-entry'); mainCard.removeEventListener('animationend', cleanup); };
        mainCard.addEventListener('animationend', cleanup);
    }
})();
...
function navigateGuide(dir) {
    ...
    if (currentGuideStep >= totalGuideSteps) { closeGuideModal(); showToast('success', "You're all set!"); return; }
    renderGuideStep();
}
function renderGuideStep() {
    ...
    for (let i = 0; i < totalGuideSteps; i++) { const dot = document.createElement('div'); dot.className = `guide-dot${i === currentGuideStep ? ' active' : ''}`; dot.addEventListener('click', () => { currentGuideStep = i; renderGuideStep(); }); guideDotsContainer.appendChild(dot); }
    guidePrevBtn.disabled = currentGuideStep === 0;
    guideNextBtn.innerHTML = currentGuideStep === totalGuideSteps - 1 ? 'Get Started <i class="fa-solid fa-check"></i>' : 'Next <i class="fa-solid fa-chevron-right"></i>';
    const active = steps[currentGuideStep]; if (active) active.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
}
